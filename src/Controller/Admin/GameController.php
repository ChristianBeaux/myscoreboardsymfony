...
class GameController extends AbstractController
{
    ...
    {
        // On ne peut pas instancier d'objet de la classe Repository, on doit les passer dans les arguments d'une m'thode d'un controleur
        // NB : pour chaque Entity créé, il y a une classe Repository qui correspond et qui permet de faire des requêtes SELECT sur la table correspondante
    ...
    * Pour accéder aux valeurs, on utilisera sur ces propriétés la méthode->get('indice')
    ...
    public function new(Request $request, EntityManagerInterface $em)
    {
        ...
        if($form->isSubmitted() && $form->isValid()){
            //la méthode paersist() prépare la requête INSERT avec les données de l'objet passé en argument
            $em->persist($jeu);
            //la méthode flush() éxécute les requêtes en attente et donc modifie la base de données
            $em->flush();
    ...
    #[Route('/admin/game/edit/{title}', name: 'app_admin_game_modifier')]
